...
export default function CodeBlock({ code: initialCode, language = 'python', filename }) {
  ...
  const langMap = { python: 'py', javascript: 'js', typescript: 'ts', bash: 'sh' };
    ...
    <div className={`code-block ide-enhanced ${hasOutput ? 'has-output' : ''}`}>
      ...
      <div className="code-block-header">
        ...
        <div className="ide-actions">
          ...
          {code !== initialCode && (
            <button className="ide-btn" onClick={handleReset} title="Reset to original">↺</button>
          )}
        ...
        {hasOutput && (
          <div className="ide-output-panel">
            <div className="ide-output-header">
              ...
              <button className="ide-clear-btn" onClick={handleClear}>✕ Clear</button>
            </div>
            ...
            {previewHTML ? (
              ...
              <div className="ide-output-body">
                {output.map((line, i) => (
                  <pre key={i} className={`ide-out-line ${line.type}`}>{line.text}</pre>
                ))}
              </div>
            )}
          </div>
